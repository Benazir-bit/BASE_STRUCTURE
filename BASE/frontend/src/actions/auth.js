...
export const login = (username, password) => dispatch => {
  ...
  axios
    .post("/apps/api/auth/login", body, config)
    .then(res => {
      dispatch({
        type: PAGE_LOADED
      });
...
export const logout = () => (dispatch, getState) => {
  axios
    .post("/apps/api/auth/logout", null, tokenConfig(getState))
    .then(res => {
      dispatch({
        type: LOGOUT_SUCCESS
      });
...
export const loadUser = () => (dispatch, getState) => {
  ...
  axios
    .get("/apps/api/auth/user", tokenConfig(getState))
    .then(res => {
      setTimeout(() => {
        dispatch({
          type: USER_LOADED,
...
export const confirmPassword = password => (dispatch, getState) => {
  ...
  axios
    .post(`/apps/api/auth/password/`, body, tokenConfig(getState))
    .then(res => {
